...
def create_new_game():
    ...
    # NEU: Config speichern (Update falls existiert)
    cursor.execute("""
        INSERT OR REPLACE INTO tournament_config 
        (game_name, match_duration, break_between_games, break_between_rounds, start_time)
        VALUES (?, ?, ?, ?, ?)
    """, (game_name, match_duration, break_between_games, break_between_rounds, start_time))
        ...
        SELECT r.*,
               (SELECT COUNT(*) FROM rankings r2 
                ...
                AND (r2.points > r.points 
                     OR (r2.points = r.points AND r2.goal_difference > r.goal_difference)
                     OR (r2.points = r.points AND r2.goal_difference = r.goal_difference 
                         AND r2.goals_for > r.goals_for))) + 1 as position
        FROM rankings r
...
# ============================================================================
# DOUBLE ELIMINATION - ZWEI-BRACKET-SYSTEM
# ============================================================================
...
def update_tournament_config(game_name):
    ...
    cursor.execute("""
        ...
        SET match_duration = ?,
            ...
            start_time = ?
        WHERE game_name = ?
    """, (match_duration, break_between_games, break_between_rounds, start_time, game_name))
...
﻿def calculate_round_robin_times(conn):
    ...
    Bracket A spielt, dann Bracket B, dann Bracket A, usw.
    """
    cursor = conn.cursor()
...
")
    ...
    # HAUPTSCHLEIFE: Pro Runde beide Brackets abwechselnd
    for round_num in range(1, 6):  # 5 Runden
        ...
        # Zeit nach Bracket B vorwärts
        current_time = add_minutes_to_time(
            current_time, 
            match_duration + break_between_games
        )
        
        # EXTRA-PAUSE zwischen Runden (außer nach letzter Runde)
        if round_num < 5:
            current_time = add_minutes_to_time(
                current_time, 
                break_between_rounds
...
")
            ...
            print("")  # Nur Leerzeile nach Runde 5
